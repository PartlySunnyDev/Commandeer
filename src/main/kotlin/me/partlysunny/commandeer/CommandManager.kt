...
class CommandManager(plugin: Plugin) : CommandExecutor, TabCompleter {
    ...
    fun register(objInst: Any) {
        ...
        JavaAccessor.getMethods(objInst.javaClass).forEach { method ->
            // Check if the method has any relevant annotations
            if (method.isAnnotationPresent(CustomCommand::class.java)) registerCommandFromMethod(method, objInst)
            else if (method.isAnnotationPresent(Subcommand::class.java)) registerSubcommandFromMethod(method, objInst)
        }
    }

    private fun registerSubcommandFromMethod(method: Method, objInst: Any): Boolean {
        // Get the annotation
        ...
        // Check the subcommand's other annotations
        registerSubcommandArgLengthBounds(method, parentCommand, subcommand)
        registerSubcommandPermissions(method, parentCommand, subcommand)
        registerSubcommandAutocomplete(method, parentCommand, subcommand)
        registerSubcommandCooldowns(method, parentCommand, subcommand)
        return false
    }

    private fun registerCommandFromMethod(method: Method, objInst: Any) {
        // Get the annotation
        ...
        registerCommandCooldowns(method, pluginCommand)
    }

    private fun registerCommandCooldowns(method: Method, pluginCommand: PluginCommand) {
    ...
    private fun registerCommandPermissions(method: Method, pluginCommand: PluginCommand) {
    ...
    private fun registerCommandArgLengthBounds(method: Method, pluginCommand: PluginCommand) {
    ...
    private fun registerSubcommandArgLengthBounds(
        method: Method,
        parentCommand: Command?,
        subcommand: Subcommand
    ) {
        if (!method.isAnnotationPresent(ArgLengthBounds::class.java)) return
        val bounds = method.getAnnotation(ArgLengthBounds::class.java)
    ...
    private fun registerSubcommandPermissions(
        method: Method,
        parentCommand: Command?,
        subcommand: Subcommand
    ) {
    ...
    private fun registerSubcommandAutocomplete(
        method: Method,
        parentCommand: Command?,
        subcommand: Subcommand
    ) {
        if (!method.isAnnotationPresent(Autocomplete::class.java)) return
    ...
    private fun registerSubcommandCooldowns(
        method: Method,
        parentCommand: Command?,
        subcommand: Subcommand
    ) {
